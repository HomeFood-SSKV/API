...
namespace Data.EF.Tool.IBusiness
{   
    public interface IiGlobalPayNeeyamoSvr
    { 
	...
	//More than crud operation it supports WCF Service implementation
        ...
        Task<List<ObserveDto>> GetObserve(CancellationToken ct = default(CancellationToken));

        Task<int> InsertObserve(ObserveDto customObject, bool commit,CancellationToken ct = default(CancellationToken));

        Task<bool> UpdateObserve(ObserveDto customObject, bool commit,CancellationToken ct = default(CancellationToken));
